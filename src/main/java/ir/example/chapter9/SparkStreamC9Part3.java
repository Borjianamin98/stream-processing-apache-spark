...
public class SparkStreamC9Part3 {

    private static final Logger log = LoggerFactory.getLogger(SparkStreamC9Part3.class);
    ...
    public static void main(String[] args) {
        ...
        Dataset<Row> rawData = sparkSession.readStream()
                .format("kafka")
                .option("kafka.bootstrap.servers", "localhost:" + 9124)
        ...
        Dataset<Row> sensorRef = sparkSession.read().json(PROJECT_RESOURCE_PATH + "\\sensorInfo.json");
        sensorRef.cache();

        Dataset<Row> sensorWithInfo = sensorRef.join(iotData, "sensorId");
        sensorWithInfo.printSchema();

        StreamingQuery query;
        try {
            query = sensorWithInfo
                    .writeStream()
