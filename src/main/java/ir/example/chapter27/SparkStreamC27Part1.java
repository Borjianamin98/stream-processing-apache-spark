...
public class SparkStreamC27Part1 {

    private static final Logger log = LoggerFactory.getLogger(SparkStreamC27Part1.class);
    ...
    public static void main(String[] args) {
        ...
        HLLAccumulator<String> hllAccumulator = new HLLAccumulator<>(12);
        javaStreamingContext.sparkContext().sc().register(hllAccumulator, "HyperLogLogAccumulator");

        JavaPairDStream<String, Long> trendStream = webLogJavaDStream
                .mapToPair(record -> new Tuple2<>(record.getHost(), 1L))
                .reduceByKey(Long::sum);

        trendStream.foreachRDD(rdd -> {
            rdd.foreach(x -> hllAccumulator.add(x._1));
            long uniqueHostCount = hllAccumulator.value();

            List<Tuple2<Long, String>> top10Hosts = rdd
                    .mapToPair(Tuple2::swap)
                    .sortByKey(false)
                    .take(10);

            System.err.println("\n=== Batch Results ===");
            System.err.println("Approximate unique hosts: " + uniqueHostCount);
            System.err.println("Top 10 hosts:");
            top10Hosts.forEach(t -> System.err.println("host: " + t._2 + ": " + t._1));

            hllAccumulator.reset();
        });
