...
public class SparkStreamC7Part5 {

    private static final Logger log = LoggerFactory.getLogger(SparkStreamC7Part5.class);
    ...
    public static void main(String[] args) {
        ...
        Dataset<Row> rankingURLStream = urlWebLogStream
                .groupBy(col("request"), window(col("timestamp"), "5 minutes", "1 minute"))
                .count();

        StreamingQuery query;
        try {
            query = rankingURLStream
                    .writeStream()
                    .queryName("urlranks")
        ...
        Dataset<Row> urlRanks = sparkSession.sql("select * from urlranks");
        urlRanks.select(col("request"), col("window"), col("count"))
                .orderBy(desc("count"))
                .show(30, false);
